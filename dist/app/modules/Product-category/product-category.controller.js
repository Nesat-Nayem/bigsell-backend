...
exports.createFashionCategoryStructure = exports.createCustomCategoryStructure = exports.createComprehensiveCategoryStructure = exports.getCategoryTemplates = exports.createCategoryFromTemplate = exports.searchCategories = exports.getCategoryBreadcrumbs = exports.getCategoriesByParent = exports.getRootCategories = exports.getCategoryTree = exports.deleteCategoryById = exports.updateCategoryById = exports.getCategoryById = exports.getAllCategories = exports.createCategory = void 0;
const product_category_model_1 = require("./product-category.model");
const product_category_validation_1 = require("./product-category.validation");
const appError_1 = require("../../errors/appError");
const cloudinary_1 = require("../../config/cloudinary");
const mongoose_1 = __importDefault(require("mongoose"));
const isRootAlias = (val) => val === null ||
    val === undefined ||
    val === "null" ||
    val === "root" ||
    val === "ROOT";
const parseParentId = (val) => {
    if (isRootAlias(val))
        return null;
    if (!val || !mongoose_1.default.Types.ObjectId.isValid(val)) {
...
const CATEGORY_TEMPLATES = {
    ...
    furniture: {
        ...
        attributes: [
            ...
            { name: "assembly_required", type: "boolean", required: false },
        ],
    },
    mobile: {
        title: "Mobile & Accessories",
        description: "Smartphones, feature phones, tablets, smartwatches and accessories",
        ...
        attributes: [
            ...
            { name: "battery_capacity", type: "text", required: false }, // e.g. "5000mAh"
            {
                name: "network",
                type: "select",
                required: false,
            ...
            { name: "camera_spec", type: "text", required: false }, // e.g. "48MP + 8MP"
            {
                name: "warranty",
                type: "select",
                required: false,
            ...
            { name: "accessories_included", type: "text", required: false }, // e.g. "Charger, Earphones, Case"
        ],
    },
};
const createCategory = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c;
    try {
        const { title, description, parentId, attributes, displayOrder, seoTitle, seoDescription, seoKeywords, icon, } = req.body;
        ...
        if (attributes) {
            try {
                ...
                return next(new appError_1.appError("Invalid attributes format", 400));
            }
        }
        // Validate parent category if provided
        if (parentId) {
            const parentCategory = yield product_category_model_1.ProductCategory.findOne({
                _id: parentId,
                isDeleted: false,
                isActive: true,
            });
        ...
        const existingCategory = yield product_category_model_1.ProductCategory.findOne({
            title,
            parentId: parentId || null,
            isDeleted: false,
        });
        if (existingCategory) {
            return next(new appError_1.appError("Category with this title already exists at this level", 400));
        }
        // Get the image URL from req.file if uploaded, or from body (pre-uploaded)
        let image = ((_a = req.file) === null || _a === void 0 ? void 0 : _a.path) || null;
        ...
        const validatedData = product_category_validation_1.categoryValidation.parse(categoryData);
        // Create a new category
        const category = new product_category_model_1.ProductCategory(validatedData);
        yield category.save();
        // Populate parent information for response
        if (parentId) {
            yield category.populate("parent");
        }
        res.status(201).json({
            success: true,
            statusCode: 201,
...
const getAllCategories = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const total = yield product_category_model_1.ProductCategory.countDocuments({ isDeleted: false });
...
const getCategoryById = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // }
        const category = yield product_category_model_1.ProductCategory.findOne({
            _id: new mongoose_1.default.Types.ObjectId(idRaw),
            isDeleted: false,
        });
        if (!category) {
...
const updateCategoryById = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (body.parentId !== undefined) {
            ...
            if (!isRootAlias(val)) {
                if (!mongoose_1.default.Types.ObjectId.isValid(val)) {
                    return next(new appError_1.appError(`Invalid parentId: ${val}`, 400));
                }
                newParent = yield product_category_model_1.ProductCategory.findOne({ _id: val, isDeleted: false });
                if (!newParent)
                    return next(new appError_1.appError("Parent category not found", 404));
        ...
        if (category.title !== undefined)
            toValidate.title = category.title;
        if (category.description !== undefined)
            toValidate.description = category.description;
        if (category.icon !== undefined)
            toValidate.icon = category.icon;
        if (category.displayOrder !== undefined)
            toValidate.displayOrder = category.displayOrder;
        if (category.seoTitle !== undefined)
            toValidate.seoTitle = category.seoTitle;
        if (category.seoDescription !== undefined)
            toValidate.seoDescription = category.seoDescription;
        if (category.seoKeywords !== undefined)
            toValidate.seoKeywords = category.seoKeywords;
        if (category.attributes !== undefined)
            toValidate.attributes = category.attributes;
        if (category.parentId === null)
            toValidate.parentId = null;
        if (category.image !== undefined)
            toValidate.image = category.image;
        if (category.isActive !== undefined)
            toValidate.isActive = category.isActive;
        // Throws if invalid
        product_category_validation_1.categoryUpdateValidation.parse(toValidate);
        ...
        if (((_b = req.file) === null || _b === void 0 ? void 0 : _b.path) && oldImage) {
            const publicId = (_c = oldImage.split("/").pop()) === null || _c === void 0 ? void 0 : _c.split(".")[0];
            if (publicId) {
                yield cloudinary_1.cloudinary.uploader.destroy(`restaurant-categories/${publicId}`);
            }
        }
        ...
        if (oldPath && newPath && oldPath !== newPath) {
            ...
            const descendants = yield product_category_model_1.ProductCategory.find({
        ...
        const updatedCategory = yield product_category_model_1.ProductCategory.findById(categoryId)
            .populate("parent")
            .populate("children");
        res.json({
            success: true,
        ...
        if ((_d = req.file) === null || _d === void 0 ? void 0 : _d.path) {
            const publicId = (_e = req.file.path.split("/").pop()) === null || _e === void 0 ? void 0 : _e.split(".")[0];
            if (publicId) {
                yield cloudinary_1.cloudinary.uploader.destroy(`restaurant-categories/${publicId}`);
            }
        }
...
const deleteCategoryById = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const children = yield product_category_model_1.ProductCategory.find({
            parentId: category._id,
            isDeleted: false,
        });
        if (children.length > 0) {
            next(new appError_1.appError("Cannot delete category with subcategories. Delete subcategories first.", 400));
            return;
        }
...
const getCategoryTree = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const parentIdRaw = req.query.parentId || null;
        ...
        const level1 = yield product_category_model_1.ProductCategory.find({
            parentId: parentId,
            isDeleted: false,
            isActive: true,
        })
        ...
        const roots = level1.map((doc) => {
            const node = Object.assign(Object.assign({}, doc), { children: [] });
        ...
        for (let depth = 2; depth <= maxDepth; depth++) {
            const parentIds = currentLevel.map((p) => p._id);
            if (!parentIds.length)
                break;
            const children = yield product_category_model_1.ProductCategory.find({
            ...
            for (const ch of children) {
                ...
                if (parentNode) {
                    const childNode = Object.assign(Object.assign({}, ch), { children: [] });
        ...
        if (!categories || categories.length === 0) {
            return next(new appError_1.appError("No categories found", 404));
        }
        res.json({
            success: true,
            statusCode: 200,
...
const getRootCategories = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const limit = parseInt(req.query.limit) || 10;
        const skip = (page - 1) * limit;
        const total = yield product_category_model_1.ProductCategory.countDocuments({
            isDeleted: false,
            isActive: true,
            parentId: null,
        });
        const totalPages = Math.ceil(total / limit);
        const categories = yield product_category_model_1.ProductCategory.find({
            isDeleted: false,
            isActive: true,
            parentId: null,
        })
...
const getCategoriesByParent = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (parentId !== null) {
            const parent = yield product_category_model_1.ProductCategory.findOne({
                _id: parentId,
                isDeleted: false,
            }).lean();
            if (!parent) {
        ...
        const total = yield product_category_model_1.ProductCategory.countDocuments(query);
        const totalPages = Math.ceil(total / limit);
        const categories = yield product_category_model_1.ProductCategory.find(query)
            .sort({ displayOrder: 1, title: 1 })
            .skip(skip)
            .limit(limit)
            .populate("parent")
...
exports.getCategoriesByParent = getCategoriesByParent;
// Get category breadcrumbs (ancestors)
const getCategoryBreadcrumbs = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const category = yield product_category_model_1.ProductCategory.findOne({
            _id: req.params.id,
            isDeleted: false,
        ...
        const breadcrumbs = yield category.getAncestors();
...
// Search categories
const searchCategories = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { query, level } = req.query;
        const statusRaw = req.query.status || undefined;
        ...
        const total = yield product_category_model_1.ProductCategory.countDocuments(searchQuery);
        const totalPages = Math.ceil(total / limit);
        const categories = yield product_category_model_1.ProductCategory.find(searchQuery)
            .sort({ displayOrder: 1, title: 1 })
            .skip(skip)
            .limit(limit)
            .populate("parent")
...
const createCategoryFromTemplate = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { categoryType, includeDefaultAttributes = true } = req.body;
        const { parentId } = req.query;
        const validatedData = product_category_validation_1.categoryTemplateValidation.parse({
            categoryType,
            includeDefaultAttributes,
        });
        const template = CATEGORY_TEMPLATES[validatedData.categoryType];
        if (!template) {
            next(new appError_1.appError("Invalid category type", 400));
            return;
        }
        ...
        if (existingCategory) {
            next(new appError_1.appError(`${template.title} category already exists at this level`, 400));
            return;
        }
        const categoryData = {
            title: template.title,
        ...
        const category = new product_category_model_1.ProductCategory(categoryData);
        yield category.save();
        res.status(201).json({
            success: true,
            statusCode: 201,
...
exports.createCategoryFromTemplate = createCategoryFromTemplate;
// Get available category templates
const getCategoryTemplates = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const templates = Object.keys(CATEGORY_TEMPLATES).map((key) => (Object.assign({ type: key }, CATEGORY_TEMPLATES[key])));
        res.json({
            success: true,
            statusCode: 200,
...
exports.getCategoryTemplates = getCategoryTemplates;
// Create comprehensive category structure like Myntra
const createComprehensiveCategoryStructure = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const createCategoryWithChildren = (categoryData_1, ...args_1) => __awaiter(void 0, [categoryData_1, ...args_1], void 0, function* (categoryData, parentId = null) {
            try {
                // Check if category already exists
                const existingCategory = yield product_category_model_1.ProductCategory.findOne({
                ...
                // Create the category
                const category = new product_category_model_1.ProductCategory({
                    title: categoryData.title,
                ...
                if (categoryData.children && categoryData.children.length > 0) {
                    for (const child of categoryData.children) {
                        yield createCategoryWithChildren(child, category._id);
                    }
                }
                return category;
            }
        ...
        for (const mainCategory of mainCategories) {
            yield createCategoryWithChildren(mainCategory);
        }
        res.status(201).json({
            success: true,
            statusCode: 201,
...
exports.createComprehensiveCategoryStructure = createComprehensiveCategoryStructure;
// Create custom category structure
const createCustomCategoryStructure = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const categoryData of categories) {
            yield createCategoryWithChildren(categoryData);
        }
        res.status(201).json({
            success: true,
            statusCode: 201,
...
exports.createCustomCategoryStructure = createCustomCategoryStructure;
// Create predefined fashion category structure (for backward compatibility)
const createFashionCategoryStructure = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // Just call the comprehensive function - it handles fashion categories too
        yield (0, exports.createComprehensiveCategoryStructure)(req, res, next);
    }
    catch (error) {
        next(error);
    }
});
exports.createFashionCategoryStructure = createFashionCategoryStructure;
